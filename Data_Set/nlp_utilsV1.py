...
def get_semantic_features(text):
    """
    Extrahiert semantische Merkmale mit TextBlob.
    
    Rückgabe:
    - Subjectivity: Wie subjektiv/objektiv ist der Text? (0.0 = Fakt, 1.0 = Meinung)
    - Caps Count: Anzahl der Großbuchstaben.
    """
...
def get_source_credibility(text):
    ...
    news_terms = ['breaking', 'news', 'reported', 'official', 'update', 'reuters', 'cnn']
    is_news_style = 1 if any(word in text_lower for word in news_terms) else 0
...
def apply_advanced_engineering(df):
    ...
    print("🔍 Schritt 5: Kontext-Extraktion...")
    ...
    print("📝 Schritt 1: Punctuation & Caps Metrics...")
    df_TEMP[['excl_count', 'ques_count', 'dot_count']] = df_TEMP[COLS['text']].apply(get_punctuation_metrics)

    # 3: ZUSÄTZLICHE SEMANTIK (Schnell, kein LLM)
    print("📝 Berechne Subjektivität...")
    df_TEMP['subjectivity'] = df_TEMP[COLS['text']].apply(lambda x: get_semantic_features(x)[0])

    # 4. GROSSSCHREIBUNG
    print("📝 Schritt 2: Großschreibung (Caps Count)...")
    df_TEMP['caps_count_raw'] = df_TEMP[COLS['text']].apply(lambda x: sum(1 for c in str(x) if c.isupper()))

    # 5. SOURCE CREDIBILITY (URLs)
    print("🔗 Schritt 3: Source Credibility...")
    df_TEMP[['has_url', 'is_news_style']] = df_TEMP[COLS['text']].apply(get_source_credibility)

    # 6. DIE KATASTROPHEN-TRIADE
    print("🎯 Schritt 4: Erzeuge Triaden-Signale (Type, Location, Time)...")
    ...
    # 7. NOISE-FILTER (eBay, Gaming, etc.)
    df_TEMP['is_noise_flag'] = df_TEMP[COLS['text']].apply(Nicht_Katastrophe).astype(int)

    # 8. FINALES SCORING (Inklusive Triple-Signal Gewichtung)
    print("📈 Schritt 7: Berechne finalen Disaster-Score...")
    ...
    # 9. ENCLAVE KI (Hardware-optimiert für MacBook Air)
    print("🤖 Schritt 8: Enclave LLM Analyse (Core-Reserve aktiv)...")
...
def plot_automated_polar_engineering(df, width=PLOT_CONFIG['width'], height=PLOT_CONFIG['height'],
    ...
    """
    Erstellt einen fächerförmig sortierten Polar-Plot.
    Features mit der höchsten Trennschärfe werden nebeneinander gruppiert,
    um ein klares Signal-Profil zu erzeugen.
    """
    # 1. RAM-schonende Kopie (Core Reserve Standard) [cite: 2025-11-21]
    df_TEMP = df.copy()
    ...
    # 3. Binäre Transformation (Präsenz-Logik)
    for col in feature_cols:
        df_TEMP[col] = (df_TEMP[col] > 0).astype(int)

    # 4. SORTIER-LOGIK (Strikt nach Katastrophe für sauberen Fächer)
    stats = df_TEMP.groupby('target')[feature_cols].mean()

    # Delta für den Barplot (Wichtigkeit)
    delta = (stats.loc[1] - stats.loc[0])

    # Sortierung für den Polar-Plot: Nur nach der Quote der Katastrophe (Klasse 1) [cite: 2025-10-14]
    sorted_features = stats.loc[1].sort_values(ascending=False).index.tolist()

    # 5. POLAR PLOT (Fächer-Form)
    df_melted = stats[sorted_features].reset_index().melt(
        id_vars='target', var_name='Feature', value_name='Quote'
    )

    # EXPLIZITE BESCHRIFTUNGEN FÜR DIE LEGENDE
    df_melted['Kategorie'] = df_melted['target'].map({1: 'Katastrophe', 0: 'Keine Katastrophe'})

    fig_polar = px.line_polar(
        df_melted, r='Quote', theta='Feature', color='Kategorie', line_close=True,
        template=PLOT_CONFIG['template'],
        color_discrete_map={'Katastrophe': color1, 'Keine Katastrophe': color2},
        title="<b>Automatisches Signal-Profil (Fokus Katastrophe)</b>"
    )
    ...
    fig_polar.update_layout(
        ...
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 1], gridcolor=PLOT_COLORS['grid']),
            angularaxis=dict(direction="clockwise", period=len(sorted_features), gridcolor=PLOT_COLORS['grid'])
        ),
    ...
    importance_df = delta.abs().sort_values(ascending=False).reset_index()
    importance_df.columns = ['Feature', 'Signal_Stärke']

    fig_bar = px.bar(
        importance_df, x='Signal_Stärke', y='Feature', orientation='h',
        title="Ranking der Trennschärfe (Delta zwischen Klassen)",
        template=PLOT_CONFIG['template'],
        color='Signal_Stärke',
    ...
    # 7. NUMERISCHE TABELLE (Unterstützungs-Regel) [cite: 2025-10-29]
    print("\n📊 SORTIERTE BASISDATEN (Präsenz-Wahrscheinlichkeit):")
    stats_out = stats[sorted_features].copy()
    stats_out.index = stats_out.index.map({1: 'Katastrophe', 0: 'Keine Katastrophe'})
    display(stats_out.round(4))

    # 8. FINALER RAM-CLEANUP (1 Core frei & RAM-Schonung) [cite: 2025-11-21, 2025-12-29]
    del df_TEMP
    del df_melted
    del importance_df
    del stats_out
...
def plot_feature_signal_ratio(df_input, feature_col, top_n=30,
                              color1=PLOT_COLORS['disaster'], color2=PLOT_COLORS['no_disaster'],
...
def run_analysis(X_train, y_train, X_test, y_test, stop_words):
    ...
    for df in [X_train_feat, X_test_feat]:
        tqdm.pandas(desc="Features", **TQDM_STYLE)
        df['llm_score'] = df[COLS['clean']].progress_apply(LLM_0_1_bool)
        df['is_llm_emergency'] = (df['llm_score'] >= 0.7).astype(int)
        df['is_noise_filter'] = df[COLS['clean']].apply(Nicht_Katastrophe).astype(int)
        ...
        df[['subjectivity', 'caps_count']] = df[COLS['clean']].apply(get_semantic_features)
        df['lemmatized_text'] = df[COLS['clean']].apply(lemmatize_text)
    ...
    feature_cols = ['lemmatized_text', 'context_trigrams', COLS['length'], 'contains_digit',
                    ...
                    'is_noise_filter', 'is_llm_emergency']
    ...
    clf_ensemble = VotingClassifier(
        estimators=[
            ('lr', LogisticRegression(solver='liblinear', n_jobs=1,
                                      **{k: v for k, v in MODEL_DEFAULTS.items() if k != 'n_jobs'})),
            ('rf', RandomForestClassifier(**MODEL_DEFAULTS)),
            ('hgb', HistGradientBoostingClassifier(random_state=MODEL_DEFAULTS['random_state']))
        ], voting='soft', weights=[2, 1, 2]
    )

    pipeline = Pipeline(
        [('prep', preprocessor), ('to_dense', FunctionTransformer(lambda x: x.toarray(), accept_sparse=True)),
         ('clf', clf_ensemble)])

    print("\n🤖 Auto-Tuning (Fokus: Recall)...")
    param_grid = {
        'prep__tfidf_main__max_features': [10000],
        'clf__lr__C': [1, 10],
        'clf__rf__n_estimators': [200],
        'clf__hgb__max_iter': [100, 200],
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        with tqdm_joblib(tqdm(total=total_fits, desc="GridSearch", **TQDM_STYLE)):
            grid_search = GridSearchCV(pipeline, param_grid, cv=3, n_jobs=MODEL_DEFAULTS['n_jobs'], scoring='recall')
            grid_search.fit(X_train_feat[feature_cols], y_train)
        if len(w) > 0: print(f"\n⚠️ {len(w)} Warnungen unterdrückt.")

    best_model = grid_search.best_estimator_
    print(f"✅ Beste Parameter: {grid_search.best_params_}")

    print("\n⛏️ Hard Example Mining (Lernen aus Fehlern)...")
    y_train_probs = best_model.predict_proba(X_train_feat[feature_cols])[:, 1]
    hard_mask = (y_train == 1) & (y_train_probs < 0.4)

    if hard_mask.sum() > 0:
        print(f" -> {hard_mask.sum()} schwierige Fälle gefunden. Trainiere nach...")
        sample_weights = np.ones(len(y_train))
        sample_weights[hard_mask] = 5.0
        best_model.fit(X_train_feat[feature_cols], y_train, clf__sample_weight=sample_weights)

    best_thresh, best_f1 = find_best_threshold(best_model, X_test_feat[feature_cols], y_test)
    ...
    plot_confusion_matrix_and_hist(y_test, y_pred, y_probs, best_thresh)
    plot_roc_curve(y_test, y_probs)

    plot_precision_recall_curve(y_test, y_probs)
    plot_confusion_matrix(y_test, y_pred)

    plot_decision_v_shape(best_model, X_test_feat[feature_cols], y_test)
    plot_error_analysis(y_test, y_probs, X_test_feat, best_thresh)

    print("\n📊 NUMERISCHE ZUSAMMENFASSUNG:")
    metrics_summary = pd.DataFrame([{
        'Modell': 'Voting Ensemble (Optimiert)',
